...
function underscoresass_scripts() {
        // Add Google Fonts Lato Noto Sans
        wp_enqueue_style( 'underscoresass-fonts', 'https://fonts.googleapis.com/css?family=Lato:400,400i,700,700i|Noto+Sans:700' );
 ...
 */
function underscoresass_content_image_sizes_attr( $sizes, $size ) {
    $width = $size[0];
...
add_filter( 'wp_calculate_image_sizes', 'underscoresass_content_image_sizes_attr', 10, 2 );
 ...
 */
function underscoresass_header_image_tag( $html, $header, $attr ) {
    if ( isset( $attr['sizes'] ) ) {
        $html = str_replace( $attr['sizes'], '100vw', $html );
    }
    return $html;
}
add_filter( 'get_header_image_tag', 'underscoresass_header_image_tag', 10, 3 );
 ...
 */
function underscoresass_post_thumbnail_sizes_attr( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'underscoresass_post_thumbnail_sizes_attr', 10, 3 );
